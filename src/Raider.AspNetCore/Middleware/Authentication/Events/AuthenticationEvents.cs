...
namespace Raider.AspNetCore.Middleware.Authentication.Events
{
	public class AuthenticationEvents
	{
		...
		public AuthenticationEvents(HttpContext context, AuthenticationOptions options)
		{
			if (options.UseWindowsAuthentication)
			{
				...
				WindowsEvents = WindowsEvents ?? new WindowsAuthenticationEvents();
			}
			...
			if (options.UseCookieAuthentication)
			{
				...
				CookieEvents = CookieEvents ?? new CookieAuthenticationEvents();
			}
			...
			if (options.UseTokenAuthentication)
			{
				...
				TokenEvents = TokenEvents ?? new JwtBearerEvents();
			}
			...
			if (options.UseRequestAuthentication)
			{
				...
				RequestEvents = RequestEvents ?? new RequestAuthenticationEvents();
			}
		}
    }
}
