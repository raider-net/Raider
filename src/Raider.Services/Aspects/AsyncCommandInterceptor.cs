...
{
	public class AsyncCommandInterceptor<TCommand, TResult, TContext, TBuilder> : ServiceInterceptorBase<TContext, TBuilder>, IAsyncCommandInterceptor<TCommand, TResult>
		...
		where TBuilder : CommandHandlerContext.Builder<TContext>
	{
		...
		public AsyncCommandInterceptor(IServiceProvider serviceProvider, ILoggerFactory loggerFactory, ILogger<AsyncCommandInterceptor<TCommand, TResult, TContext, TBuilder>> logger)
		...
		public async Task<ICommandResult<bool>> InterceptCanExecuteAsync(ITraceInfo previousTraceInfo, IAsyncCommandHandler<TCommand, TResult> handler, TCommand command, ICommandInterceptorOptions? options, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				try
				{
					...
					if (handlerResult == null)
						throw new InvalidOperationException($"Handler {handler.GetType().FullName}.{nameof(handler.CanExecuteAsync)} returned null. Expected {typeof(ICommandResult<TResult>).FullName}");
		...
		public async Task<ICommandResult<TResult>> InterceptExecuteAsync(ITraceInfo previousTraceInfo, IAsyncCommandHandler<TCommand, TResult> handler, TCommand command, ICommandInterceptorOptions? options, CancellationToken cancellationToken)
		{
			long callStartTicks = StaticWatch.CurrentTicks;
			long callEndTicks;
			decimal methodCallElapsedMilliseconds = -1;
