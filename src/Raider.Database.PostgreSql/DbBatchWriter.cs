...
namespace Raider.Database.PostgreSql
{
	public abstract class DbBatchWriter<T> : BatchWriter<T>, IDisposable
	{
			...
			: base(options, errorLogger)
		{
			...
			BulkInsert = new BulkInsert(options.ToBulkInsertOptions());
		}

		public abstract IDictionary<string, object?>? ToDictionary(T obj);

		protected override async Task WriteBatch(IEnumerable<T> batch, CancellationToken cancellationToken = default)
		{
			...
			await BulkInsert.WriteBatch(rows, _connectionString);
		}
	}
}
