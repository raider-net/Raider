...
using Raider.Reflection.Caching;

//http://fasterflect.codeplex.com/documentation
...
namespace Raider.Reflection.Emitter
{
	internal abstract class BaseEmitter
	{
        ...
        internal Delegate GetDelegateForStruct()
        {
            var action = cache.Get(CallInfo);
            if (action == null)
            {
                Method = CreateDynamicMethod();
                Generator = new EmitHelper(Method.GetILGenerator());
                action = CreateDelegateForStruct();
                cache.Insert(CallInfo, action, CacheStrategy.Temporary);
            }
            return action;
        }
        ...
        protected internal abstract Delegate CreateDelegateForStruct();
        ...
        protected void LoadInnerStructToLocalForStruct(byte localPosition)
        {
            Generator
                //.castclass(Constants.StructType) // (ValueTypeHolder)wrappedStruct
                //.callvirt( StructGetMethod ) // <stack>.get_Value()
                  .unbox(CallInfo.TargetType) // unbox <stack>
                  //.stloc(localPosition) // localStr = <stack>
                  //.ldloca_s(localPosition) // load &localStr
                  ;
        }
