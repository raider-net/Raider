...
namespace Raider.Localization
{
	public class ResxBuilder
    {
		...
		private root _root;
		private List<ResxData> _dataList;
		...
		public ResxBuilder(string resxFilePath, bool createIfDoesNotExists = true)
		{
			...
			{
				_resxFilePath = resxFilePath;
				_root = XmlSerializerHelper.ReadFromXml<root>(_resxFilePath);

				if (_root == null)
					throw new ArgumentException(nameof(resxFilePath));
			}
		...
		{
			_root.Items = new List<object>(_root.Items.Where(item => !(item is rootData)));
			_dataList.Clear();
			return this;
		}
